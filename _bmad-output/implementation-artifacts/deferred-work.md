...
- **DEF-V8-AXE3-1 — Régex carbone trop large : matche `bilan ESG` / `bilan financier`** — `_FORCE_CARBON_FINALIZE_RE = r"\b(finalise|...).{0,40}\b(bilan|carbone)\b"` accepte `valide ce bilan ESG`, `confirme mon bilan financier`. Limité par routing préalable (carbon_node atteint UNIQUEMENT si `_route_carbon=True`) et par la pré-condition `applicable.issubset(completed)`, mais durcissement souhaitable. **Path** : restreindre à `\b(bilan(\s+(?:carbone|GES|d['e]missions))?|carbone|empreinte\s+carbone)\b` + ajouter test négatif `"valide mon bilan ESG"`. Coût : 15 min. [backend/app/graph/nodes.py]
- **DEF-V8-AXE3-2 — NFC normalization Unicode sur `HumanMessage.content`** — Inputs mobile/copy-paste peuvent être NFD-décomposés (`é` ≠ `é`) → regex échoue silencieusement. Théorique mais possible. **Path** : `content = unicodedata.normalize("NFC", content)` en préalable des deux helpers `_should_force_*`. Coût : 10 min. [backend/app/graph/nodes.py]
- **DEF-V8-AXE3-3 — Filtre interrogatif manquant** — `Tu fais quoi avec mon score ?` matche actuellement (`fais` + `score`). Limité par routing + verbe imperative requis, mais false-positive possible. **Path** : post-filtre excluant sentences finissant par `?` ou commençant par `comment|pourquoi|quand|quoi|qu'est-ce`. Coût : 15 min. [backend/app/graph/nodes.py]
- **DEF-V8-AXE3-4 — Logger sans `extra={"user_id":..., "conversation_id":...}`** — `logger.info("Forced tool invocation: ...")` ne permet pas de filtrer par utilisateur/conversation en cas d'incident. Pas de PII leak (contenu non loggé), mais traçabilité opérationnelle dégradée. **Path** : ajouter `extra={"metric": "tool_force_invocation", "node_name": "credit"|"carbon", "user_id": str(state.get("user_id"))}` aux deux `logger.info`. Coût : 10 min. [backend/app/graph/nodes.py]
- **DEF-V8-AXE3-5 — Race condition concurrence (pré-existant)** — Deux requêtes SSE concurrentes mêmes `conversation_id` (double-clic, retry frontend) → deux forçages simultanés, deux exécutions du tool, race possible avant que la garde DB (`status='completed'` carbon, `age_days < 30` credit) n'absorbe la 2e. Pré-existant à V8-AXE3 (LangGraph MemorySaver sans verrou par thread). **Path** : `SELECT ... FOR UPDATE` côté `complete_assessment` / `generate_credit_score` (story dédiée). Coût : 1-2 h. **Trigger** : pic d'incidents `IntegrityError` ou doubles credit_scores. [backend/app/modules/carbon/service.py + backend/app/modules/credit/service.py]
- **DEF-V8-AXE3-6 — Multimodal `HumanMessage.content` list[dict]** — Si user upload image+texte, `content` est `list[dict]` au lieu de `str` → forçage désactivé silencieusement (défense en profondeur via `isinstance(content, str)`). Comportement non documenté. **Path** : `_extract_text(content)` joignant les `{"type":"text"}` de la liste, ou documenter explicitement le fallback dans Design Notes. Coût : 20 min. [backend/app/graph/nodes.py]

## Deferred from Vague 8 batch architectural — multi-goal split (2026-04-28)

Stratégie : 5 specs séquentielles, ordre AXE3 → AXE2 → AXE1 → AXE4 → AXE5. AXE3 traité en premier (spec `spec-v8-axe3-routing-credit-carbon-finalize.md`). Les 4 axes ci-dessous sont reportés à des cycles bmad-quick-dev ultérieurs, à exécuter dans cet ordre.

- **DEF-V8-AXE2 — Validation `generate_action_plan` relâchée + fallback template** — Régression BUG-V6-005 + BUG-V7-007 + BUG-V7.1-009 (9 erreurs HTTP 500 cycle PLAN). **Path** : seuil minimum ≥5 actions au lieu de ≥10, mode incrémental avec persistance partielle, JSON parse tolérant (repair trailing commas/quotes), template `backend/app/templates/action_plan_default.json` 10 actions ESG/Carbone/Financement par secteur en fallback après 3 retries < 5 actions. **Files** : `backend/app/graph/tools/action_plan_tools.py` + nouveau `backend/app/templates/action_plan_default.json`. **Trigger** : après merge AXE3.
- **DEF-V8-AXE1 — Extraction profil hybride LLM + regex déterministe** — Régression BUG-V7-001 + BUG-V7.1-001 (sector/city/name jamais extraits). **Path** : prompt few-shot 3 exemples obligatoires dans `backend/app/prompts/profiling.py`, nouveau `backend/app/services/profile_extraction.py` avec dictionnaires SECTORS_FR + COUNTRIES_UEMOA/CEDEAO et regex employee_count/name suffix légal, appelé EN COMPLÉMENT du tool LLM (pas en remplacement) : si tool retourne args null → fallback regex détermine + UPDATE BDD. **Files** : `backend/app/prompts/profiling.py` + nouveau `backend/app/services/profile_extraction.py` + `backend/app/graph/tools/profiling_tools.py`. **Trigger** : après merge AXE2.
- **DEF-V8-AXE4 — Mapping subcategory carbone exhaustif backend** — Régression BUG-V7-005 + BUG-V7.1-006 + BUG-V7.1-007 (subcategory=null systématique, "diesel"→"gasoline" mapping incorrect). **Path** : nouveau `backend/app/services/carbon_mapping.py` avec dictionnaire `SUBCATEGORY_SYNONYMS` exhaustif (agriculture/energy/transport/waste, ~30 entrées FR canonical), `resolve_subcategory(category, raw_text)` avec match exact lowercase puis fuzzy Levenshtein ≤2, intégration dans `save_emission_entry` (carbon_tools.py) en fallback si subcategory=null/inconnu, erreur runtime listant options possibles si non résolu. Tests régression 30 cas FR. **Files** : nouveau `backend/app/services/carbon_mapping.py` + `backend/app/graph/tools/carbon_tools.py` + `backend/tests/test_carbon_mapping.py`. **Trigger** : après merge AXE1.
- **DEF-V8-AXE5 — Cluster backend non-LLM (validators REST + dashboard + gamif + setup.sh)** — 4 sous-patchs indépendants : (5a) `field_validator` strip+blank-rejection sur `CompanyProfileUpdate.company_name/city/country/sector` (BUG-V7-002, BUG-V7.1-002), (5b) `dashboard_summary` inclut `status='in_progress'` avec `is_partial=True` au lieu de filtrer `completed` only (BUG-V7.1-014), (5c) listeners SQLAlchemy `@event.listens_for(CarbonAssessment, 'after_insert'/'after_update')` pour gamification au lieu d'écouter événements applicatifs (BUG-V7-010, BUG-V7.1-013), (5d) `setup.sh` racine racine projet automatisant `brew install pango cairo gdk-pixbuf libffi glib` macOS pour WeasyPrint (BUG-V7-003, BUG-V7.1-012). **Files** : `backend/app/schemas/company.py` + `backend/app/modules/dashboard/service.py` + nouveau `backend/app/modules/gamification/hooks.py` + nouveau `setup.sh` + `README.md`. **Trigger** : après merge AXE4. Stratégie de découpage interne : peut être re-split en 4 sous-specs si volume excessif au moment de l'exécution.
...
## Deferred from BUG-V6 batch runtime guards persistence tools (2026-04-28)

- **DEF-BUG-V6-CREDIT-RISKLEVEL — `score.risk_level` n'existe pas sur le modèle CreditScore** — détecté pendant l'investigation BUG-V6. `backend/app/graph/tools/credit_tools.py:57` (et `:88` dans `get_credit_score`) référencent `score.risk_level` mais la colonne réelle du modèle est `confidence_label` (Enum very_low/low/medium/good/excellent). Les tests masquent le bug via `_make_credit_score` qui set `risk_level` sur un MagicMock. À la première vraie exécution avec un objet SQLAlchemy réel, `AttributeError`. **Trigger** : avant tout E2E credit en environnement non-mocké. **Path** : remplacer `score.risk_level` par `score.confidence_label.value` (avec mapping FR si besoin) ou ajouter une property `@property risk_level(self) -> str` sur le modèle qui mappe `confidence_label` → libellé FR. Mettre à jour `_make_credit_score` pour cesser d'injecter `risk_level`. Source : investigation BUG-V6, marqué `# TODO DEF-BUG-V6-CREDIT-RISKLEVEL` dans `credit_tools.py`. [backend/app/graph/tools/credit_tools.py:57,90 + backend/app/models/credit.py + backend/tests/test_tools/test_credit_tools.py:25]

- **DEF-BUG-V6-CARBON-UNIQUE — Ajouter UNIQUE constraint BDD sur (assessment_id, category, subcategory)** — la garde runtime de `save_emission_entry` deduplique en lecture-puis-écriture, vulnérable à une race condition sous appels concurrents (2 SSE turns parallèles → 2 INSERT). **Trigger** : si BUG-V6-002 réapparaît malgré la garde runtime. **Path** : migration Alembic ajoutant `UniqueConstraint("assessment_id", "category", "subcategory", name="uq_carbon_entry_dedup")` sur `carbon_emission_entries`, puis convertir le SELECT-then-INSERT en INSERT...ON CONFLICT...DO UPDATE (postgres upsert). Source : Edge case hunter review batch BUG-V6 finding #2.

- **DEF-BUG-V6-CARBON-PRIVATE-IMPORT — `_compute_total_emissions` importé depuis service** — `backend/app/graph/tools/carbon_tools.py` importe la fonction privée `_compute_total_emissions` du service via un import inline. Reaching across module boundaries to call a private function. **Trigger** : refactor du service carbon. **Path** : exposer une fonction publique `recompute_total_emissions(db, assessment_id)` ou regrouper la garde runtime + recompute dans un helper service public consommé par le tool. Source : Blind hunter review batch BUG-V6 finding #5.

- **DEF-BUG-V6-CARBON-CANONICAL-KEY — Normaliser subcategory aliasing avant dedup** — `save_emission_entry` deduplique par `factor_key` canonique (résultat du fallback EMISSION_FACTORS). Si le LLM passe `subcategory="diesel"` (free-text) puis `subcategory="diesel_transport"` (clé exacte), 2 lignes seront créées car les `factor_key` diffèrent après fallback. **Trigger** : si des doublons réapparaissent sur des sous-catégories à libellé variable. **Path** : map alias→canonical_key dans `app.modules.carbon.emission_factors`, normaliser avant tout SELECT/INSERT. Source : Edge case hunter review batch BUG-V6 finding #1.

- **DEF-BUG-V6-CREDIT-MIN-AGE-CONFIG — `_CREDIT_SCORE_MIN_AGE_DAYS` doit être configurable** — actuellement constante module-level (30 jours), aucune surcharge env/admin. Bloque les scénarios QA / re-test sans modification de code. **Trigger** : besoin légitime de regénérer un score < 30 jours en environnement non-prod. **Path** : exposer `settings.credit_score_min_age_days` (Pydantic Settings, default 30, override `.env`), wrapper admin endpoint pour bypass ponctuel avec audit log. Source : Blind hunter review batch BUG-V6 finding #9.

- **DEF-BUG-V6-DEDUP-MARKER-CONTRACT — Test invariant prompt/code sur le marqueur [DEDUP]** — `carbon_tools.py` injecte le préfixe `[DEDUP]` dans le message de retour, et `prompts/carbon.py` instruit le LLM à reconnaître ce marqueur. Aucun test n'asserte que les deux restent alignés. **Trigger** : avant un refactor du message d'erreur. **Path** : test d'invariant statique (`tests/test_prompts/test_dedup_marker_contract.py`) qui vérifie que la string `[DEDUP]` apparaît à la fois dans `carbon_tools.py` (sortie) et dans `prompts/carbon.py` (instruction). Source : Blind hunter review batch BUG-V6 finding #12.

## Deferred from V8-AXE5 backend cluster non-LLM (2026-04-29)

- **DEF-V8-AXE5-1 — Commit hook prematuré dans transaction caller** — `safe_check_and_award_badges` appelle `check_and_award_badges` qui fait `await db.commit()` ligne 127, alors que les 4 call-sites (`carbon_tools.py:255-256`, `esg_tools.py:227-228`, `application_tools.py:87-88`, `financing_tools.py:202-203`, `credit/service.py:756-757`) ne committent pas explicitement avant le hook. Risque marginal : si le tool raise APRES le hook, les modifs d'assessment sont deja commitees ensemble — le pattern est coherent avec `update_action_item:617-625`. Trigger : si un tool effectue d'autres operations apres `complete_assessment` qui doivent etre rollbackable. Path : ajouter `await db.commit()` explicite avant chaque hook OU refactorer `check_and_award_badges` pour `flush()` au lieu de `commit()` et laisser le caller commiter. Source : code-reviewer R1-2 + python-reviewer R3-2.

- **DEF-V8-AXE5-2 — Stale `.env` non detecte par setup.sh** — Si `backend/.env` existe deja avec des cles obsoletes ou manquantes par rapport a `.env.example`, `setup.sh` skippe sans warning. Trigger : onboarding sur un repo clone partiellement configure. Path : ajouter une etape de comparaison `diff .env.example .env` avec warning sur cles manquantes, sans ecraser. Source : python-reviewer R3-6.

- **DEF-V8-AXE5-3 — Deferred imports + bare except masque circular import** — Les 4 tool call-sites + `credit/service.py` importent `safe_check_and_award_badges` dans le corps de fonction au lieu du top-level. Le bare `except Exception` du wrapper cache une `ImportError` silencieusement. Trigger : refactor introduisant un cycle. Path : convertir en imports module-level avec `TYPE_CHECKING` ou restructurer la dependance `action_plan ↔ carbon`. Pattern volontaire actuel (cf. `action_plan/service.py:622-623`). Source : code-reviewer R1-1 + python-reviewer R3-8.

- **DEF-V8-AXE5-4 — `variation_percent` calcule contre `previous` in_progress** — `_get_carbon_summary` lignes 134-140 calculent une variation percentuelle entre `latest.completed` et `previous.in_progress` (total partiel par definition). Affichage potentiellement trompeur sans indicateur. Trigger : quand un utilisateur a un bilan completed N et un brouillon N-1 abandonne. Path : skip variation si `previous.status != CarbonStatusEnum.completed` OU ajouter flag `previous_partial: bool` dans le dict. Source : python-reviewer R3-7.

- **DEF-V8-AXE5-5 — Tri secondaire status dans `_get_carbon_summary` ambigu** — Apres simplification (review R3-3), seul `updated_at desc` est utilise. Si 2 in_progress meme annee, le plus recemment touche gagne, mais aucune garantie qu'il s'agit du bilan "principal". Patch actuel suffisant pour le cas courant. Trigger : si UI affiche un bilan abandonne plutot que le bilan en cours. Path : ajouter colonne `is_active` ou status enum etendu. Source : python-reviewer R3-3 (residu apres patch).
