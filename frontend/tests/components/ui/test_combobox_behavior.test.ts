...
describe('ui/Combobox : AC2 mode single + multiple', () => {
  ...
  it('multiple : render badges pour chaque valeur preselectionnee', () => {
    render(Combobox, {
      props: {
        modelValue: ['sn', 'ci'],
        options: [...PAYS_OPTIONS],
...
describe('ui/Combobox : AC3 IME composition guard (piege #38)', () => {
  it("pendant compositionstart → input n'est pas filtre (cote composant)", async () => {
    const user = userEvent.setup();
    render(Combobox, {
      props: {
        modelValue: null,
    ...
    // Simule frappe intermediaire composition (ex. `e` avant finalisation `é`).
    // Utilise fireEvent.update pour synchroniser v-model (happy-dom/vue).
    await fireEvent.update(input, 'e');
    // Assertion stricte Leçon 21 : Burkina Faso n'a aucun `e`/`é` dans son
    // label — s'il est visible malgre searchTerm='e', c'est que notre guard
    // IME isComposing a bien neutralise le filter cote composant.
    // (Reka UI a son filter interne desactive via filter-function=() => true.)
    expect(
      screen.queryByRole('option', { name: /burkina faso/i }),
    ).not.toBeNull();
    expect(
      screen.queryByRole('option', { name: /côte d'ivoire/i }),
    ).not.toBeNull();
  });

  it('apres compositionend → filter declenche normalement', async () => {
    const user = userEvent.setup();
    render(Combobox, {
      props: {
        modelValue: null,
    ...
    const opt = await screen.findByRole('option', { name: /sénégal/i });
    expect(opt).toBeDefined();
    expect(screen.queryByRole('option', { name: /côte d'ivoire/i })).toBeNull();
  });
});
