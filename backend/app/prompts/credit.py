...
Le score affiche a l'utilisateur n'est valide que si persiste via `generate_credit_score`. Ne JAMAIS repondre textuellement avec des valeurs (score combine, solvabilite, impact, niveau de risque) sans avoir appele le tool. La BDD est l'unique source de verite — le dashboard et le chat doivent presenter exactement le meme score.
- **CONTROLE RUNTIME** : `generate_credit_score` est idempotent sur 30 jours. Si un score recent existe deja, le tool retourne `ERREUR : score credit deja genere ce mois (...)`. Dans ce cas, ne reessaie PAS de regenerer ; appelle plutot `get_credit_score` pour le consulter et le presenter.
- INTERDIT : produire un score textuel different de celui en BDD. INTERDIT : "j'estime votre score a..." sans tool.
...
def build_credit_prompt(
    ...
    prompt = build_prompt(module="credit", base=base)
