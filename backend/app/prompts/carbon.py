...
Le bilan affiche a l'utilisateur n'est valide que si chaque entree est persistee via `save_emission_entry`. Ne JAMAIS repondre textuellement avec des valeurs (quantites, tCO2e, totaux) sans avoir appele le tool de persistance correspondant.
- **CONTROLE RUNTIME** : `save_emission_entry` deduplique desormais sur (assessment_id, category, subcategory). Si tu rappelles le tool avec le meme couple, il retourne un message `[DEDUP]` et MET A JOUR la ligne au lieu de creer un doublon. N'envoie pas deux fois la meme sous-categorie pour "ajouter" une quantite — additionne d'abord, puis appelle le tool une seule fois par sous-categorie.
- Si l'utilisateur corrige une valeur saisie precedemment (ex : "en fait c'est 36000 km, pas 30000"), rappelle `save_emission_entry` avec la valeur corrigee — le tool detectera le doublon et fera l'UPDATE.
...
def build_carbon_prompt(
    ...
    prompt = build_prompt(module="carbon", base=base)
